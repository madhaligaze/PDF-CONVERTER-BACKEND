...
BLOCKS: tuple[str, ...] = (
    ...
    "registries",
    # Финансы — управленческий учёт компании: счета, операции, отчёты. Право
    # отдельное и самое дорогое в списке: здесь видно движение всех денег и
    # остатки на счетах, то есть больше, чем в любом другом разделе.
    "finance",
)
...
# Разделы, которые учётке сотрудника не выдаются вообще — только администратору.
#
# Не «опасные», а *неразрезаемые*: журнал операций и лист ОМиП не несут ни
# отдела, ни сотрудника, поэтому сузить их до области видимости нечем. Отдать
# такой раздел «с областью ОБО» значит отдать его целиком — с ФОТ, фамилиями и
# движением денег всей компании. Пока в этих листах не появится ключ отдела,
# галочка в карточке сотрудника не должна создавать вид, что резать умеет.
ADMIN_BLOCKS: tuple[str, ...] = ("journal", "sales")

#: Что можно отметить в карточке сотрудника.
EMPLOYEE_BLOCKS: tuple[str, ...] = tuple(b for b in BLOCKS if b not in ADMIN_BLOCKS)
