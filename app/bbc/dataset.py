...
MASTER_COLUMNS: tuple[Column, ...] = (
    ...
    # В сентябре 2026 книга обзавелась вторым сальдо. Прежние «Сальдо Начало» и
    # «Сальдо Конец» на позициях 22 и 46 переименовали в «(АВР)», а в конец
    # листа (53 и 54) дописали пару «(без АВР)».
    #
    # Дашборд встал, и это правильно: «Сальдо Начало» стало нечётко совпадать с
    # двумя денежными колонками сразу, а выбирать наугад, из какой брать
    # деньги, нельзя — цифры выглядели бы как цифры. Отказ читать лист был
    # единственным честным поведением.
    #
    # Берём пару «(АВР)»: это те же самые колонки, что читались всегда, — они
    # остались на своих местах, им только уточнили название, — и цифры на
    # экране от переименования не меняются. Пара «(без АВР)» новая и стоит в
    # дописанном блоке; если считать нужно по ней, это отдельное решение и
    # отдельная правка, а не догадка разбора.
    #
    # Старое написание остаётся в списке: на копии книги без этой правки стоят
    # тесты и локальные прогоны.
    Column(
        "saldo_start",
        "Сальдо Начало",
        ("Сальдо Начало (АВР)", "Сальдо Начало"),
        hint=22,
    ),
    ...
    # Пара к «Сальдо Начало» выше — там же и объяснение выбора «(АВР)».
    Column(
        "saldo_end",
        "Сальдо Конец",
        ("Сальдо Конец (АВР)", "Сальдо Конец"),
        hint=46,
    ),
    Column(
        "status",
